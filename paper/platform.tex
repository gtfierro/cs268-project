...
reset_sock = storm.net.udpsocket(6666, function(msg, ip)
    if not heard_reset then
        heard_reset = true
        if msg == "RAND" then
            wait = wait + math.random(10)
        end
        storm.os.invokeLater(wait*storm.os.SECOND, storm.os.reset)
        cord.new(function()
            for i=1,5 do
                cord.await(storm.os.invokeLater, math.random(3)*storm.os.SECOND)
                storm.net.sendto(reset_sock, "RAND", "ff02::1", 6666)
            end
        end)
    end
end)
...
\begin{luacode}
t1=storm.os.invokeLater(5*storm.os.SECOND, storm.os.reset) 
